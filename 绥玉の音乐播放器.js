...
// @namespace    https://github.com/SuiYu-233// @match        :///*
...
(function() {
    'use strict';
// 启动日志
console.log('🎵 [播放器-修复版] 脚本开始执行');

// 全局错误捕获
window.addEventListener('error', function(e) {
    console.error('❌ 播放器脚本错误:', e.message, '位于:', e.filename, ':', e.lineno);
});
...
function initPlayer() {
    ...
    function loadTrack(index, shouldPlay) {
        ...
        if (shouldPlay && track.src) {
            ...
            if (playPromise !== undefined) {
                playPromise.catch(e => {
                    console.log('⏸️ 自动播放被阻止，需用户交互。错误:', e);
                });
            }
        }
    }
    ...
    function renderPlaylist() {
        ...
        TRACKS.forEach((track, index) => {
            ...
            `;
            trackEl.addEventListener('click', () => loadTrack(index, true));
